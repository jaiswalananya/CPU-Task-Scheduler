...
vector<pair<char, int>> algorithms; // pair<algorithm_id, quantum>
vector<tuple<string,int,int>> processes; // (process_name, arrival, service)
...
void parse_algorithms(string algorithm_chunk)
{
    ...
    while (stream.good())
    {
        ...
        if (temp_str.empty()) continue;  // skip empty entries
        ...
        if (algorithm_id == '2') {  
            ...
            quantum = temp_str.size() >= 1 ? stoi(temp_str) : -1;
        }
        
        if (algorithm_id == '1' || algorithm_id == '2' || algorithm_id == '3') {
